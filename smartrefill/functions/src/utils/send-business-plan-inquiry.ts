...
export async function sendBusinessPlanInquiry(data: InquiryProp) {
  try {
    const {
      firstName,
      lastName,
    ...
    const businessType = (business as any)?.type || "Unknown";
    ...
    sendSmtpEmail.subject = "Business Plan Inquiry for Smart Refill";
    ...
    logger.info(`Business Plan Inquiry from ${email}`);
    return { success: true };
  } catch (error: any) {
